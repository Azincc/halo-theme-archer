...
class MetaInfo {
    constructor(metaName, labelsContainer, postContainer) {
        ...
        if (!this.isValid) {
            console.warn(`Sidebar meta "${metaName}" containers not found. Skipping initialization.`);
            return;
        }
    ...
    _changeFocus(label) {
        ...
        const currFocus = this.labelsContainer.getElementsByClassName('sidebar-label-focus');
    ...
    tryInit(postsArr) {
        if (!this.isValid || this.isInited || Object.prototype.toString.call(postsArr) === '[object Null]') {
            return;
        }
...
class SidebarMeta {
    ...
    addTab(para) {
        this.tabCount++;
        const newMeta = new MetaInfo(para.metaName, para.labelsContainer, para.postsContainer);
        if (!newMeta.isValid) {
            return;
        }
