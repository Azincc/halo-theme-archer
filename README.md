...
* **2024-12-27**：修复暗黑模式切换失败的问题：
  - **问题**：点击顶部的切换按钮时，由于暗黑样式表路径错误（引用了 `/css/dark.css`），导致样式文件加载失败。
  - **解决方案**：更新 `assets/scripts/theme.js`，根据 `siteMeta.root` 组装 `assets/css/dark.css` 的正确路径，并处理根路径缺少斜杠的情况，保证暗黑样式表正常加载和卸载。
